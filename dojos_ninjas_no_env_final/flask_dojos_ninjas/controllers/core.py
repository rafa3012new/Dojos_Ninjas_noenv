...
def mostrar_dojos():
  ...
  print(dir(Dojo.get_all()[0]),flush=True)
...
@app.route('/mostrarninjasdojo/<int:id>')
def mostrar_ninjas_dojo(id):

   data = {"id" : id}

   resultado = Dojo.Get_Dojo_With_Ninjas(data)

   if resultado == None:
     flash("Error al obtener los registros de ninjas por dojo","error")
   else:
     flash(f"Se ha cargado los registros de ninjas para el dojo {resultado.name} con exito","success")


   return render_template('submain_ninjas.html',dojo=resultado)
