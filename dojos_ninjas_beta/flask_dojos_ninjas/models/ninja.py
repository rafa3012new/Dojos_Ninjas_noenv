...
class Ninja:
    ...
    # ahora usamos métodos de clase para consultar nuestra base de datos
    @classmethod
    def get_by_first_name_like(cls,fisrt_name):
        #armar la consulta con cadenas f
        query = f"SELECT * FROM ninjas where first_name like %(first_name)s;"
        #armar el diccionario data con solo parte del campo first_name usando like
    ...
    # ahora usamos métodos de clase para consultar nuestra base de datos
    @classmethod
    def get_by_last_name_like(cls,name):
        #armar la consulta con cadenas f
        query = f"SELECT * FROM ninjas where last_name like %(last_name)s;"
        #armar el diccionario data con solo parte del campo first_name usando like
    ...
    @classmethod
    def save(cls, data):
        query = f"INSERT INTO users (first_name , last_name , age , created_at, updated_at ) VALUES ( %(first_name)s , %(last_name)s , %(age)s , NOW() , NOW() );"
    ...
    @classmethod
    def update(cls, data):
        query = f"UPDATE ninjas SET first_name = %(first_name)s , last_name = %(last_name)s, age = %(age)s, created_at = NOW(), updated_at = NOW() WHERE id = %(id)s;"
        # data es un diccionario que se pasará al método de guardar desde server.py
